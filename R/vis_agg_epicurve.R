...
#'
#' @param dat A data frame containing number of cases/deaths per day to be
#'   visualized.
#'
#' @param start_date The start date of the visualization. Defaults to NULL,
#'   which means that the earliest date in the data will be used.
#'
#' @param end_date The end date of the visualization. Defaults to `Sys.Date()`,
#'   which means that the current date will be used.
#'
#' @param var A variable to be used for the y-axis. Defaults to `new_cases`.
#'
#' @param date_var A variable to be used for the x-axis. Defaults to
#'   `date_reported`.
#'
#' @param by_week A logical value indicating whether to aggregate the data by
#'   week. Defaults to FALSE.
#'
#' @param plot_title The plot title. Defaults to NULL, which means that no title
#'   will be used.
#'
#' @param fill_by The variable to be used for the fill aesthetic. Defaults to
#'   "who_region_long".
#'
#' @param facet_by The variable to be used for faceting. Defaults to NULL, which
#'   means that no faceting will occur.
#'
#' @param free_y A logical value indicating whether the y-axis should be scaled
#'   independently for each facet. Defaults to TRUE.
#'
#' @param col_pal The color palette to be used. Defaults to
#'   `outbreak_pal("qual1")`.
#'
#' @param facet_cols The number of columns in the facet grid. Defaults to 2.
#'
#' @param return_data A logical value indicating whether to return the data once
#'   processed. Defaults to FALSE.
#'
#' @return If return_data is TRUE, returns a data frame. Otherwise, it returns a
#'   ggplot object.
#'
...
vis_agg_epicurve <- function(dat,
  ...
  dat <- dat %>%
    group_by(
      who_region, {{ date_var }}, who_region_long,
      across(any_of(fill_by)), across(any_of(facet_by))
    ) %>%
  ...
  if (by_week) {
    dat <- dat %>%
      ...
      summarise(
        across(contains("new"), \(x) sum(x, na.rm = TRUE)),
        across(contains("total"), \(x) max(x, na.rm = TRUE))
      ) %>%
      ungroup()
  }
  ...
  if (fill_by == "who_region_long")
    out <- out + guides(fill = guide_legend(nrow = 2, byrow = TRUE))
