...
#' This function extends `cut` functionality to allow for several labelling styles
#'
#' @param x a `numeric` vector containing numbers to be binned
#'
#' @param breaks a numeric vector of breaks to be used for cut. There is no need to add Inf.
#' For percentages, provide numbers as proportions.
#'
#' @param mode one of "int" (a vector of integer values, e.g. age, cases),
#' "dbl" (a vector of floating point numbers with decimals e.g. attack rate),
#' "pct" (a vector of percentages, expressed as decimals),
#'  or "pct_change" (a vector of percentages, specifically when showing percent change between two time points).
#'
#'
#' @author HLS
#'
...
bin_numbers <- function(x,
  ...
  cut(x, breaks = breaks, labels = labs, include.lowest = TRUE, right = ifelse(mode == "int", FALSE, TRUE))
